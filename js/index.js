...
function initNav() {
    ...
    //拿到吸顶导航
    let ONavFloat=document.querySelector(".nav-floating");
    //拿到原航道高度
    ...
    window.onscroll=function () {
        ...
        if (offsetY>navHeight){
            //显示吸顶导航
            ONavFloat.style.top="0";
        }else {
            //隐藏吸顶导航
            ONavFloat.style.top="-100px";
        }
    }
}
...
function waterfall() {
    ...
    for(let i=0;i<oItems.length;i++){
        ...
        if (i<cols){
            item.style.position="";
            rowHeight.push(item.offsetHeight+20);
        }else {
            //如果不是第一行按照指定排版
            //找到第一行最矮的元素
            let minHeight=Math.min.apply(this,rowHeight);//这个方法会把最小的值返回给我
            //找到第一行最矮的那个索引
            let minIndex=rowHeight.findIndex(function (value) {
                return value===minHeight;
            });//findIndex方法有个回调函数,只要遍历到value值,就会返回最小的给我
            //跟据索引取出最爱的元素
            let minItem=oItems[minIndex];
            //获取到最矮的那个元素的offsetleft
...
function initBottomData() {
    ...
    let obj=intiFooterJsonData();
    let html = template('example', obj);
